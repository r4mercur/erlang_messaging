...
-export([create_conversation/1, get_conversation/1, get_user_conversations/1]).
...
handle_call({get_conversation_by_name, RoomName}, _From, #state{pool_name = Pool} = State) ->
    Result = poolboy:transaction(Pool, fun(Worker) ->
        Query = "SELECT id, kind FROM conversation WHERE meta->>'room_name' = $1",
        case gen_server:call(Worker, {query, Query, [RoomName]}) of
            {ok, _Columns, [{Id, Kind}]} ->
                {ok, #{id => Id, kind => Kind}};
            {ok, _Columns, []} ->
                {error, not_found};
